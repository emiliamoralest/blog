...
```
And we run the pipeline with:
```bash
pixi run nextflow run script4.nf
```
Or `pixi run nextflow run script4.nf --reads 'data/ggal/*_{1,2}.fq'` for overwriting the initial command and reading more files.
